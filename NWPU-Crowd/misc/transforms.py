...
class RandomCropWithInstance(object):
    ...
    def __call__(self, img, mask, dst_size=None):
        ...
        x1list = np.arange(0, w - tw, 200)
        y1list = np.arange(0, h - th, 200)

        max_sum = 0.0
        x1_best = random.randint(0, w - tw)
        y1_best = random.randint(0, h - th)
        for x1 in x1list:
            for y1 in y1list:
                x1 = int(x1)
                y1 = int(y1)
                crop_mask = mask.crop((x1, y1, x1 + tw, y1 + th))
                sum = np.sum(np.array(crop_mask))
                if sum > max_sum:
                    max_sum = sum
                    x1_best = x1
                    y1_best = y1
