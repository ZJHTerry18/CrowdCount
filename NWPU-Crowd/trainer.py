...
class Trainer():
    def __init__(self, cfg_data, pwd):
        ...
        self.net = CrowdCounter(cfg.GPU_ID,self.net_name, cfg.GS_SIGMA).cuda()
        self.optimizer = optim.Adam(self.net.CCN.parameters(), lr=cfg.LR, weight_decay=1e-4)
        # self.optimizer = optim.SGD(self.net.parameters(), cfg.LR, momentum=0.95,weight_decay=5e-4)
        self.scheduler = StepLR(self.optimizer, step_size=cfg.NUM_EPOCH_LR_DECAY, gamma=cfg.LR_DECAY)          

        self.train_record = {'best_mae': 1e20, 'best_mse':1e20, 'best_nae':1e20, 'best_mae_10': 1e20, 'best_nae_10':1e20,'best_model_name': ''}
    ...
    def train(self): # training for all datasets
        ...
        for i, data in enumerate(self.train_loader, 0):
            ...
            if (i + 1) % cfg.PRINT_FREQ == 0:
                ...
                loguru.logger.info( '[ep %d][it %d][loss %.6f][lr %.4f][%.2fs]' % \
                        (self.epoch + 1, i + 1, loss.item(), self.optimizer.param_groups[0]['lr']*10000, self.timer['iter time'].diff) )
                loguru.logger.info( '        [cnt: gt: %.1f pred: %.2f]' % (gt_map[0].sum().data/self.cfg_data.LOG_PARA, pred_map[0].sum().data/self.cfg_data.LOG_PARA) )           
    ...
    def validate(self):
        ...
        mses = AverageMeter()
        naes = AverageMeter()
        maes_10 = AverageMeter()
        naes_10 = AverageMeter()
        ...
        for vi, data in enumerate(self.val_loader, 0):
            ...
            with torch.no_grad():
                ...
                if gt_count >= 10:
                    naes_10.update(s_nae)
                if gt_count < 10:
                    maes_10.update(s_mae)
        ...
        overall_mae_10 = maes_10.avg
        overall_nae_10 = naes_10.avg
        ...
        self.writer.add_scalar('mae_for_uncrowded', overall_mae_10, self.epoch + 1)
        self.writer.add_scalar('nae_for_crowded', overall_nae_10, self.epoch + 1)

        self.train_record = update_model(self.net,self.optimizer,self.scheduler,self.epoch,self.i_tb,self.exp_path,self.exp_name, \
            [overall_mae, overall_mse, overall_nae, overall_mae_10, overall_nae_10, loss],self.train_record,self.log_txt)

        print_NWPU_summary(self.exp_name, self.log_txt,self.epoch,[overall_mae, overall_mse, overall_nae, overall_mae_10, overall_nae_10, loss],self.train_record,c_maes,c_mses, c_naes)
